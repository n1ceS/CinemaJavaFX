...
public class AdminAddMovieController implements Initializable {
    ...
    @FXML
    public AnchorPane addMovieAnchorPane;
    @FXML
    ...
    public void setMovieManagerController(MoviesManagerController moviesManagerController) {
    ...
    public void addMovie(MouseEvent mouseEvent) throws IOException, SQLException {
        if(validateInput()) {
    ...
    public boolean validateInput() {
            ...
            if(file ==null) {
                displayMessage(imageAndTextAreaErrorLabel, "Nie wybrano obrazu!");
                return false;
            }
        return true;
    }
    ...
    public void backToMovies(MouseEvent mouseEvent) {
        ...
        StackPane dashBoardStackPane = (StackPane) addMovieAnchorPane.getParent();

        //animacja
        Timeline timeline = new Timeline();
        KeyValue keyValue = new KeyValue(addMovieAnchorPane.translateYProperty(), scene.getHeight(), Interpolator.EASE_OUT);
        KeyFrame keyFrame = new KeyFrame(Duration.seconds(0.3), keyValue);
        timeline.getKeyFrames().add(keyFrame);
        timeline.setOnFinished(event1 -> {
            dashBoardStackPane.getChildren().remove(addMovieAnchorPane);
        });
        timeline.play();
    }
}
